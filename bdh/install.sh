...
# Usage: curl -fsSL https://raw.githubusercontent.com/juanre/beadhub/main/bdh/install.sh | bash
#
# Security note: For maximum security, download and inspect the script first:
#   curl -fsSL https://raw.githubusercontent.com/juanre/beadhub/main/bdh/install.sh > install.sh
...
install_with_go() {
    ...
    if go install github.com/$REPO/bdh@latest; then
...
main() {
    ...
    echo "  1. Install Go from https://go.dev/dl/"
    echo "  2. Run: go install github.com/$REPO/bdh@latest"
    echo ""
    exit 1
}
